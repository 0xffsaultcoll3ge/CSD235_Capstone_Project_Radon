...
def best_model_path(event: str, model_dir: str, threshold=None) -> str:
    try:
        model_dir = os.path.join(model_dir, event, str(threshold)) if threshold else os.path.join(model_dir, event)
        if not os.path.exists(model_dir) or not os.listdir(model_dir):
            raise FileNotFoundError(f"No models found in: {model_dir}")

        best_model = max(os.listdir(model_dir), key=lambda model: float(re.search(r'\d+\.\d+', model).group()))
...
class NHLModel:
    ...
    def get_team_prediction(self, home: str, away: str):
        ...
        try:
            ...
            dmat = self.convert_to_dmatrix(match_df)
