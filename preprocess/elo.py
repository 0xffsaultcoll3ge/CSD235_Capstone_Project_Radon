...
class Elo:
    ...
    def update_ratings(self, winner: str, loser: str, decay: float, margin: float, inflation: float) -> None:
        ...
        self.rating_dict[winner] = self.rating_dict[winner] + self.k *margin*inflation* (1 - expected_result)
        self.rating_dict[loser] = self.rating_dict[loser] + self.k *margin*inflation* (-1 + expected_result)
